...
**Scope:** Sub-project C of the mesa project. Follows A-scrape + A-ingest + B-brain + D-calendar (all merged to `main`). Delivers objective 4 (step-by-step assignment guidance), objective 5 (test-prep questions), objective 6 (book summaries), plus ad-hoc course Q&A. No scheduling, no calendar.
...
| `testprep` format           | ~15 questions: recall / application / 1-2 mini-case, difficulty-labelled. Each followed by a `<details><summary>Model answer</summary>…</details>` block (model answer or rubric). Default scope = sessions conducted so far (`brain_next_session.sessionsConducted`); `/mesa:ares-brain-testprep <course> <N>-<M>` overrides.                                   |
...
| Book summary → brain        | `book-summary` writes only to `study/`. A new `ingest` rule normalizes `courses/<slug>/study/book-*-summary.md` → `normalized/mesa:ares-brain-book-summary-<bookSlug>.md` (`type: book-summary`). `ingest` remains the sole writer of `normalized/`; `brain-index` picks the new doc up on its next run, making book summaries searchable by `ask` / `testprep`. |
...
### `ask` — `/mesa:ares-brain-ask <course> "<question>"`
...
### `assignment-help` — `/mesa:ares-brain-assignment-help <course> "<assignment title>"` (or an assignment id)
...
### `testprep` — `/mesa:ares-brain-testprep <course> [N-M]`
...
### `book-summary` — `/mesa:ares-brain-book-summary [course]`
...
- `_diff_write(... type_="book-summary", source=f"study/{path.name}", title=<from frontmatter or stem>, ...)` → `normalized/mesa:ares-brain-book-summary-<slug>.md` (agent-written artifacts carry no `title`, so the fallback strips the `book-`/`-summary` affixes from the stem: `book-lean-startup-summary` → `lean-startup`).
...
- **`_normalize_study_book_summaries`:** a `study/book-x-summary.md` → a `normalized/mesa:ares-brain-book-summary-x.md` (`type: book-summary`) after `step_ingest`; second run unchanged → skipped; delete the `study/` file → the normalized doc orphan-deleted; the `book-summary` doc is then indexed by `build_index` and returned by `query(type="book-summary")`.
- **MCP `brain_get` tool:** temp `ARES_BRAIN_HOME`, stub the spawn, assert argv + summary parsing; a missing doc → `{error}` surfaced.
- **Skills:** no automated test (Claude-Code-executed). The live run is the check.
- **Live run** (final task): `/mesa:ares-brain-ask` a real course question (check citations point at real docs); `/mesa:ares-brain-assignment-help` for a real unsubmitted assignment (check the artifact + that it's a skeleton not a draft); `/mesa:ares-brain-testprep` for one course (check ~15 Qs + collapsible answers); drop a real recommended book PDF into `inbox/<slug>/books/`, `/mesa:ares-brain-course-ingest`, `/mesa:ares-brain-book-summary`, then `/mesa:ares-brain-course-ingest` again and confirm the summary is searchable. Record in `docs/lms-api.md`.
...
3. `ares-brain-ask` skill + `/mesa:ares-brain-ask` command.
4. `ares-brain-assignment-help` skill + `/mesa:ares-brain-assignment-help` command.
5. `ares-brain-testprep` skill + `/mesa:ares-brain-testprep` command.
6. `ares-brain-book-summary` skill + `/mesa:ares-brain-book-summary` command + `_normalize_study_book_summaries` ingest rule + live run.

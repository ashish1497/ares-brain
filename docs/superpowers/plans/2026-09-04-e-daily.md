...
**Goal:** A launchd job runs `claude -p /mesa:ares-brain-course-daily` at 06:00 that scrapes, syncs the calendar, and writes `daily/<date>.md` + a macOS notification — built on a new deterministic `daily_brief` data layer.

**Architecture:** `scraper/daily_brief.py` (pure) assembles today's classes + pre-reads, assignments due ≤72h, and what changed since the last scrape. CLI + MCP tool expose it. The `ares-brain-course-daily` skill orchestrates `all` → `calendar-sync` → `daily_brief` → digest → notification. `/mesa:ares-brain-daily-setup` installs the launchd plist with a scoped `--allowedTools` allowlist.
...
## Task 3: `ares-brain-course-daily` skill + command

**Files:**

- Create: `skills/mesa:ares-brain-course-daily/SKILL.md`
- Create: `commands/mesa:ares-brain-course-daily.md`

No unit tests (Claude-executed). Verified in the Task 5 live run.

- [ ] **Step 1: Write `skills/mesa:ares-brain-course-daily/SKILL.md`**
...
- [ ] **Step 2: Write `commands/mesa:ares-brain-course-daily.md`**
...
Invoke the `ares-brain-course-daily` skill. This is the same routine the 6 AM launchd job runs;
...
## Task 4: `/mesa:ares-brain-daily-setup` + `/mesa:ares-brain-daily-uninstall` commands

**Files:**

- Create: `commands/mesa:ares-brain-daily-setup.md`
- Create: `commands/mesa:ares-brain-daily-uninstall.md`

- [ ] **Step 1: Write `commands/mesa:ares-brain-daily-setup.md`**
...
- [ ] **Step 2: Write `commands/mesa:ares-brain-daily-uninstall.md`**
...
```bash
git add -A && git commit -m "E task 4: /mesa:ares-brain-daily-setup + /mesa:ares-brain-daily-uninstall commands
...
Run `/mesa:ares-brain-course-daily` in this session. Confirm `daily/<today>.md` exists with all four
sections filled and a grounded (not generic) mindset paragraph, and the macOS
notification fired.

- [ ] **Step 3: launchd**

`/mesa:ares-brain-daily-setup`. Then `launchctl start co.mesa.ares-brain.daily`. Wait ~2 min, then
...
`README.md`: new `## Daily job` section — what it does, `/mesa:ares-brain-daily-setup` to install,
`daily/<date>.md` + notification output, `/mesa:ares-brain-daily-uninstall` to stop, and that it's
independent of `/mesa:ares-brain-course-scrape` (which stays manual/on-demand). Note transcription is
never part of it.
...
**Spec coverage:** `daily_brief.py` 3 blocks + `scrapeStale` (§architecture) → T1. CLI (§CLI) → T1 S6. MCP tool (§MCP tool) → T2. skill 5 steps + failure-tolerance (§skill) → T3. `/mesa:ares-brain-daily-setup` scoped allowlist + plist template + `/mesa:ares-brain-daily-uninstall` (§commands) → T4. live run + `--allowedTools` validation + README (§live run) → T5. import-isolation test (§testing) → T1 S2.

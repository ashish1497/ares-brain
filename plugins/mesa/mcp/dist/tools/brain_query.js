...
export const brainQueryTool = {
    ...
    async handler(args = {}) {
        ...
        const p = {};
        if (args.course)
            p.course = args.course;
        if (args.type)
            p.type = args.type;
        if (args.sessionMin !== undefined)
            p.session_min = args.sessionMin;
        if (args.sessionMax !== undefined)
            p.session_max = args.sessionMax;
        if (args.dueBefore)
            p.due_before = args.dueBefore;
        if (args.text)
            p.text = args.text;

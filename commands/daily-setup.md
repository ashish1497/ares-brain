...
`claude plugin marketplace add ./course-agent` and
`claude plugin install mesa-course-agent@mesa-course-agent-local`. The plugin is a
...
3. Write `~/Library/LaunchAgents/co.mesa.course-agent.daily.plist` with EXACTLY this
   ...
   <plist version="1.0"><dict>
     ...
     <key>ProgramArguments</key><array>
       ...
       <string>-p</string><string>/mesa-course-agent:course-daily</string>
       <string>--permission-mode</string><string>acceptEdits</string>
       <string>--allowedTools</string>
       <string>Bash(cd:*),Bash(uv run python lms_scrape.py:*),Bash(osascript:*),Write(daily/**),Read(courses/**),mcp__plugin_mesa-course-agent_mesa-course-agent__daily_brief,mcp__plugin_mesa-course-agent_mesa-course-agent__calendar_sync,mcp__plugin_mesa-course-agent_mesa-course-agent__scrape,mcp__plugin_mesa-course-agent_mesa-course-agent__ingest</string>
     </array>
...
4. `launchctl unload ~/Library/LaunchAgents/co.mesa.course-agent.daily.plist 2>/dev/null; launchctl load ~/Library/LaunchAgents/co.mesa.course-agent.daily.plist`
   ...
   `launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/co.mesa.course-agent.daily.plist`
   and `launchctl bootout gui/$(id -u)/co.mesa.course-agent.daily`.
...
(the repo), so they only line up while `COURSE_AGENT_HOME` is unset (its default). If the
user has pointed `COURSE_AGENT_HOME` elsewhere, widen those two entries to the absolute
